...
{
    public class OrderPrintingService : IOrderPrintingService
    {
        private EservicesdbContext _db;
        public OrderPrintingService(EservicesdbContext db)
        {
        ...
        public async Task<Control<OrderItemDTO>> Get(int start, int take, string filter, string order, bool active)
        {
            ...
            var repos = _db.OrderItem.Where(m => m.RoleId == EOrderTypes.OrderPrinting.Description()).AsQueryable();
            if (active)
            {
        ...
        {
            entity.RoleId = EOrderTypes.OrderPrinting.Description();
            if (entity.Id == 0)
            {
